...
void Renderer::Render(const Camera& camera, glm::vec3 m_SphereColor) {
	Ray ray;
	...
	for (uint32_t y = 0; y < m_FinalImage->GetHeight(); y++) 
	{
		for (uint32_t x = 0; x < m_FinalImage->GetWidth(); x++) 
		{
			...
			glm::vec4 color = TraceRay(ray, m_SphereColor);
...
glm::vec4 Renderer::TraceRay(const Ray& ray, glm::vec3 sphereColor)
{
	float radius = .5;
